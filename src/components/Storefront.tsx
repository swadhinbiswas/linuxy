  ...
  const fetchData = async () => {
    try {
      ...
      if (source === "appimagehub") {
        ...
        const response = await fetch("https://api.github.com/search/repositories?q=topic:appimage&sort=stars&order=desc&per_page=100");
    ...
    <div style={{ paddingBottom: "40px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <div>
          ...
          <p style={{ color: "var(--text-secondary)" }}>
            {source === "appimagehub" ? "Browsing AppImageHub Catalog" : "Browsing Popular GitHub AppImage Projects"}
          </p>
        </div>
        <div style={{ display: "flex", gap: "10px", background: "var(--bg-input)", padding: "5px", borderRadius: "8px", border: "1px solid var(--border-color)" }}>
          <button 
            onClick={() => setSource("appimagehub")}
            style={{ 
              display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", border: "none", borderRadius: "6px", cursor: "pointer",
              background: source === "appimagehub" ? "var(--bg-card)" : "transparent", color: "var(--text-primary)",
          ...
          <button 
            ...
            style={{ 
              display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", border: "none", borderRadius: "6px", cursor: "pointer",
              background: source === "github" ? "var(--bg-card)" : "transparent", color: "var(--text-primary)",
      ...
      <div style={{ position: "relative", marginBottom: "20px", maxWidth: "400px" }}>
        ...
        <input 
          ...
          style={{ 
            width: "100%", padding: "10px 10px 10px 35px", background: "var(--bg-input)", border: "1px solid var(--border-color)", borderRadius: "6px", color: "var(--text-primary)", fontSize: "16px"
          }}
        />
      </div>

      {loading && <div style={{ textAlign: "center", padding: "40px", color: "var(--text-secondary)" }}>Loading {source === "github" ? "GitHub projects" : "Hub catalog"}...</div>}
      {error && <div style={{ color: "var(--danger-color)", padding: "20px", background: "rgba(255,0,0,0.1)", borderRadius: "8px" }}>{error}</div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px" }}>
        {filteredApps.map((app, idx) => {
          const githubLink = app.links?.find(l => l.type.toLowerCase() === "github" || l.type.toLowerCase() === "homepage");
            ...
            <div key={idx} style={{ 
              ...
              <div style={{ display: "flex", alignItems: "center", marginBottom: "15px" }}>
                {app.icons && app.icons.length > 0 ? (
                  <img 
                    src={app.icons[0].startsWith("http") ? app.icons[0] : `https://appimage.github.io/database/${app.icons[0]}`} 
                    alt={app.name} 
                ...
                <div style={{ overflow: "hidden" }}>
                  <div style={{ fontWeight: "bold", fontSize: "18px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: "var(--text-primary)" }}>{app.name}</div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                <button 
                  ...
                  onClick={async () => {
                    try {
                      if (source === "github" && app.github_url) {
                        ...
                        const asset = release.assets?.find((a: any) => a.name.toLowerCase().endsWith(".appimage"));
                  ...
                  style={{ 
                    flex: 1, background: "#4caf50", color: "#fff", border: "none", padding: "10px", borderRadius: "6px", cursor: "pointer", 
                ...
                {app.github_url && (
                  <button 
                    ...
                    style={{ 
                      background: "var(--bg-input)", color: "var(--text-primary)", border: "1px solid var(--border-color)", width: "40px", borderRadius: "6px", cursor: "pointer", 
